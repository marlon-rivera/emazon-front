...
import { of } from 'rxjs';
import { ASC_ORDER, CRITERIA_ARTICLE_NAME, SIZE_PAGE } from '@/app/shared/utils/api.constants';
...
describe('ListArticlesPageComponent', () => {
  ...
  it('should load articles on init', () => {
    ...
    expect(component.paginationInfo).toEqual(mockPaginationResponse.paginationInfo);
  });
  ...
  it('should check if mobile on window resize', () => {
  ...
  describe('getVisiblePages', () => {
    ...
    it('should handle case when totalPages is exactly maxVisiblePages + 1', () => {
      component.paginationInfo.totalPages = 6;
      component.currentPage = 2;
      
      const visiblePages = component.getVisiblePages();
      
      expect(visiblePages).toEqual([0, 1, 2, 3, 4, -1, 5]);
    });
  });
});